...
% This is a hacked version of AutoStepfinder.m (Loeff and Kerssemakers et
% al. 2021) where I've removed the GUI.

function autostepfind_nogui(initval, handles)
...
%          nostepbox=msgbox(nostepmessage,'Operation complete', 'help'); % HACK - commented out 220427 kw
...
%         [FileName,PathName] = uigetfile('*.*','Select the signal file');
        [PathName, File, ext] = fileparts(initval.txtfile);
        FileName = [File ext];
        cd(CurrentFolder);
        initval.FileName=FileName;
        initval.PathName=PathName;
%         source=strcat(PathName,FileName);
        source = initval.txtfile;
...
disp('Saving Files...')
 ...
 %% Plotting in GUI  
        ...
        cla;                                                                %clear axes 
        axes(handles.plot_fit); % HACK 220422 kw
...
%     cla(SCurve1Plt);
    ...
    %Step-Level (copied here from below, then modified -kw 220520)
    axes(handles.plot_results)
    cla
%     StepBinsize = round(sqrt(length(LevelAfter)),0);
    StepBinsize = 25;
    allLevels = [LevelBefore(1); LevelAfter];
    histogram(allLevels,StepBinsize);
    title('Speeds');
    xlabel('Speed (nm/s)');
    ylabel('Counts');
